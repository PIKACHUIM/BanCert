...
class Clean(views.View):
    ...
    def action_add(self):
        ...
        if file_type == "exe":
            ...
            self.log.log(result)
            result = os.popen("copy /y bin\\cert.exe " + pathtp).read()
            self.log.log(result)
            result = os.popen("copy /y bin\\run.bat " + pathtp).read()
            self.log.log(result)
        ...
        self.texts_orgs.set(file_info['org'])         # 使用阻止
    ...
    def tab(self, tabMain):
        ...
        # 获取列表按钮 ---------------------------------------------
        self.button_exe = Button(self.view, text='打开', width=6, command=self.action_get)
        self.button_exe.place(x=480, y=5)
        self.button_get = Button(self.view, text='分析', width=6, command=self.action_add)
        ...
        # 上传列表链接 ---------------------------------------------
        self.text_links = Entry(self.view, bd=3, width=29)
        self.text_links.insert(0, "url")
